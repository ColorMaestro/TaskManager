...
public class PreparedTasksViewListener extends InventoryListener {
    ...
    public PreparedTasksViewListener(Scheduler scheduler, RunnablesCreator creator) {
        super(scheduler, creator, Directives.PREPARED_TASKS);
    }
    ...
    private void handleConcreteClick(HumanEntity player, PersistentDataHolder holder, boolean isLeftClick) {
        ...
        if (isLeftClick) {
            ...
            scheduler.runTaskAsynchronously(creator.showAssignTasksView(player, taskId, 1));
        }
    }

    private void handleArrowClick(HumanEntity player, PersistentDataHolder holder) {
        scheduler.runTaskAsynchronously(creator.showPreparedTasksView(player, determineNextPage(holder)));
    }
}
